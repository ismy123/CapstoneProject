...
public class RoadGen : MonoBehaviour // 끝없는 길 만들기
{
    ...
    {
        nextStep = nextStep + 5;
            newRoad = Instantiate(PrefabsRoad[Random.Range(0, 2)], new Vector3(startPos.position.x, startPos.position.y, transform.position.z + nextStep * 2), Quaternion.identity) as GameObject; // 길 생성
    }
}